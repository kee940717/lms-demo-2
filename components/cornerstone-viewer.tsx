...
export function CornerstoneViewer({
  ...
  const initializeViewer = useCallback(async () => {
    ...
    try {
      ...
      element.addEventListener('cornerstoneviewportchanged', (e) => {
        console.log('Viewport changed event triggered:', e);
        handleViewportChanged(e);
      });
      element.addEventListener('cornerstonetoolsmeasurementadded', (e) => {
        console.log('Measurement added event triggered:', e);
      });
      element.addEventListener('cornerstonetoolsmeasurementmodified', (e) => {
        console.log('Measurement modified event triggered:', e);
      });
  ...
  const setTool = useCallback(
    (toolName: any) => {
      ...
      try {
        const primaryMouseTools = ['Pan', 'Length', 'RectangleRoi', 'EllipticalRoi', 'Wwwc'];
        primaryMouseTools.forEach(tool => {
          cornerstoneTools.setToolPassive(tool);
        });
        cornerstoneTools.setToolActive(toolName, { mouseButtonMask: 1 });

        // Manually set cursor style
        const cursorMap = {
          wwwc: 'crosshair',
          pan: 'move',
          zoom: 'zoom-in',
        ...
        if (elementRef.current) {
          elementRef.current.style.cursor = cursorMap[toolName.toLowerCase()] || 'default';
        }
